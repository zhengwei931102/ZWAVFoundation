...
- (IBAction)rotateCameraBtnFunc:(id)sender
{
    ...
    _isRotatingCamera = NO;
    //zw前置摄像头镜像设置
    AVCaptureConnection *fileConnection=[self.videoOutput connectionWithMediaType:AVMediaTypeVideo];
    fileConnection.videoOrientation = AVCaptureVideoOrientationLandscapeRight;
    if ((toChangePosition == AVCaptureDevicePositionFront || toChangePosition == AVCaptureDevicePositionUnspecified) && fileConnection.supportsVideoMirroring) {
        fileConnection.videoMirrored = YES;
    }else{
        fileConnection.videoMirrored = NO;
    }
...
- (void)initAVCaptureSession
{
    ...
    fileConnection.videoOrientation = AVCaptureVideoOrientationLandscapeRight;
    if ((currentPosition == AVCaptureDevicePositionFront || currentPosition == AVCaptureDevicePositionUnspecified) && fileConnection.supportsVideoMirroring) {
        fileConnection.videoMirrored = YES;
    }else{
        fileConnection.videoMirrored = NO;
    }
...
- (void)takePhotos:(UITapGestureRecognizer *)tapGestureRecognizer
{
    ...
    if ((currentPosition == AVCaptureDevicePositionFront || currentPosition == AVCaptureDevicePositionUnspecified) && captureConnection.supportsVideoMirroring) {
...
- (void)previewPhotoWithImage:(UIImage *)image
{
    ...
    if (self.shootingOrientation == UIDeviceOrientationLandscapeRight)
    {
        if (currentPosition == AVCaptureDevicePositionFront || currentPosition == AVCaptureDevicePositionUnspecified) {
            finalImage = [UIImage imageWithCGImage:image.CGImage scale:1.0 orientation:UIImageOrientationUpMirrored];
        }else{
            finalImage = [self image:image rotation:UIImageOrientationDown];
        }
    ...
    {
        if (currentPosition == AVCaptureDevicePositionFront || currentPosition == AVCaptureDevicePositionUnspecified) {
            finalImage = [UIImage imageWithCGImage:image.CGImage scale:1.0 orientation:UIImageOrientationDownMirrored];
        }else{
            finalImage = [self image:image rotation:UIImageOrientationUp];
        }
    ...
    {
        if (currentPosition == AVCaptureDevicePositionFront || currentPosition == AVCaptureDevicePositionUnspecified) {
            finalImage = [UIImage imageWithCGImage:image.CGImage scale:1.0 orientation:UIImageOrientationRightMirrored];
        }else{
            finalImage = [self image:image rotation:UIImageOrientationLeft];
        }
    ...
    {
        if (currentPosition == AVCaptureDevicePositionFront || currentPosition == AVCaptureDevicePositionUnspecified) {
            finalImage = [UIImage imageWithCGImage:image.CGImage scale:1.0 orientation:UIImageOrientationLeftMirrored];
        }else{
            finalImage = [self image:image rotation:UIImageOrientationRight];
        }
